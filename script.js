...
function addNota() {
  ...
  let colunaRefrencia = document.querySelector("#media0");
  let novaColuna = document.createElement("th");

  novaColuna.innerText = `Nota ${qtdNotas}`;
  document.querySelector("#linha0").insertBefore(novaColuna, colunaRefrencia);
  ...
  for (let aluno = 1; aluno <= qtdAlunos; aluno++) {
    ...
    novaColuna = dadoNota;
    colunaRefrencia = document.querySelector(`#media${aluno}`).parentElement;
    document
      .querySelector(`#linha${aluno}`)
      .insertBefore(novaColuna, colunaRefrencia);
  }
}
...
function delNota() {
  if (qtdNotas > 1) {
    ...
    for (let aluno = 1; aluno <= qtdAlunos; aluno++) {notaDeletar = document.querySelector(`#nota${qtdNotas}-${aluno}`).parentElement;
...
function calcular() {
  ...
  for (let aluno = 1; aluno <= qtdAlunos; aluno++) {
    ...
    for (let nota = 1; nota <= qtdNotas; nota++) {notaParcial = Number(document.getElementById(`nota${nota}-${aluno}`).value);
      notaSoma = notaSoma + notaParcial;
    }
    
    mediaFinal = notaSoma / qtdNotas;
    if (mediaFinal >= 5) {situacao = "Aprovado"; situacaoCor = "green";}
     else if (mediaFinal >= 4,5 && mediaFinal < 5) {situacao = "Recuperação"; situacaoCor = "yellow";}
     else if (mediaFinal < 4,5) {situacao = "Reprovado"; situacaoCor = "red";
    }
